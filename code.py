...
def Executer():
    ...
    dfc = dfc.transpose()
    dfc.to_csv(r"C:\Users\maha chavva\internship\College_data.csv", index=True)

    stu = pd.DataFrame(student)
    stu = stu.transpose()
    stu = stu.sort_values(stu.columns[1], axis=0)
    stu = stu.set_index(stu.columns[1]).reset_index()
    stu.to_csv(r"C:\Users\maha chavva\internship\studentpreferencedata1.csv", index=False)
    ...
    for i in college.keys():
        ...
        df.to_csv(r"C:\Users\maha chavva\internship\{}.csv".format(i), index=False)
